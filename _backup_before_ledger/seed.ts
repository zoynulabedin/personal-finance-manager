...
async function main() {
  // Hard stop: every delete below is scoped to the demo account, but seeding a
  // live database is still almost never what you want.
  ...
  const hashedPassword = await bcrypt.hash(process.env.SEED_PASSWORD || "DemoPassword123", 12);
  ...
  // Clear existing records **for the demo user only**. These used to be
  // unscoped deleteMany({}) calls, so one seed run wiped every account's data.
  await prisma.expense.deleteMany({ where: { userId: user.id } });
  await prisma.income.deleteMany({ where: { userId: user.id } }); // cascades donations
