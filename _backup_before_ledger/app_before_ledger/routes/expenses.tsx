...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (intent === "create") {
    ...
    const applyBalance = deductBalance && refs.bankAccountId !== null;
    ...
    await prisma.$transaction(async (tx) => {
      await tx.expense.create({
        data: {
          ...
          balanceApplied: applyBalance,
          date: expenseDate,
          note,
        },
      });

      if (applyBalance && refs.bankAccountId) {
        // Atomic decrement. Read-then-write loses one of two concurrent
        // submits because both read the same starting balance.
        await tx.bankAccount.update({
          where: { id: refs.bankAccountId },
  ...
  if (intent === "delete") {
    ...
    await prisma.$transaction(async (tx) => {
      const existing = await tx.expense.findFirst({
        where: { id, userId },
        select: { id: true, amount: true, bankAccountId: true, balanceApplied: true },
      });
      if (!existing) return;

      // Put the money back. Deleting used to leave the deduction in place
      // forever, so every removed expense permanently understated the balance.
  ...
  if (intent === "update") {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // Reverse whatever the old row applied, then apply the new figures.
      // Editing used to change the amount without touching the balance at all.
      if (existing.balanceApplied && existing.bankAccountId) {
        await tx.bankAccount.update({
          where: { id: existing.bankAccountId },
          data: { currentBalance: { increment: existing.amount } },
                    ...
                    <div className="flex items-center justify-center gap-2">
                      <button
                        onClick={() => {
                          ...
                          setNote(item.note || "");
                          setDeductBalance(item.balanceApplied);
                        }}
