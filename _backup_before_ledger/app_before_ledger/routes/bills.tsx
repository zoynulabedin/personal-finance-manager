...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  // The page is titled "monthly bills" but used to load every bill ever
  // created, so the three summary cards were lifetime totals.
...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (intent === "toggle_paid") {
    ...
    // Derive the next state from the stored row rather than trusting a hidden
    // field, so a stale form can't flip an already-updated bill back.
    ...
    await prisma.bill.update({
      ...
      data: {
        paid: !bill.paid,
        paidDate: !bill.paid ? new Date() : null,
      },
    });
