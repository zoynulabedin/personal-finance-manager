...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (intent === "pay_donation") {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const accountId = await assertOwnedAccount(tx, userId, bankAccountId);
      if (!accountId) return { error: "নির্বাচিত ব্যাংক একাউন্টটি বৈধ নয়।" };

      // Guarded update: a double submit can only win the race once, so the
      // account is never debited twice for the same donation.
      const marked = await tx.donation.updateMany({
        where: { id: donation.id, paid: false },
        data: { paid: true, paidDate: new Date(), bankAccountId: accountId },
      });
      ...
      await debit(tx, {
        ...
        type: "DONATION",
        description: `১% দান: ${donation.income.title}`,
