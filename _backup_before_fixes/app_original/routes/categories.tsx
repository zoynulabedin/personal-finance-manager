...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (intent === "delete") {
    ...
    if (id) {
      // Check if expenses exist for this category for current user
      const count = await prisma.expense.count({ where: { categoryId: id, userId } });
      if (count > 0) {
        return { error: "এই ক্যাটাগরিতে খরচ যুক্ত রয়েছে, তাই মুছে ফেলা সম্ভব নয়।" };
      }
